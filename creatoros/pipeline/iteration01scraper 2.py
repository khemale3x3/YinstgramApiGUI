...
def diagnose_exception(e, driver=None, context=""):
    """
    Build a rich diagnostic report for an exception.

    Returns a dict with:
      - exc_type / exc_message   : what actually raised
      - location                : file:line:function where it was raised
                                   (your code, not chromedriver's binary)
      - traceback_text           : full Python traceback (multi-line)
      - driver_state             : best-effort snapshot of the browser
                                    session at the moment of failure
    """
    exc_type = type(e).__name__
    exc_message = str(e) or "(no message)"

    tb = traceback.extract_tb(e.__traceback__)
    if tb:
        last_frame = tb[-1]  # the frame closest to where it was raised
    ...
    if driver is not None:
        # Every one of these can itself throw if the session is dead —
        # that's useful info too, so we record the failure instead of
        # silently swallowing it.
        try:
            driver_state["current_url"] = driver.current_url
        except Exception as inner:
            driver_state["current_url"] = f"<unavailable: {inner}>"

        try:
            driver_state["title"] = driver.title
        except Exception as inner:
            driver_state["title"] = f"<unavailable: {inner}>"

        try:
            driver_state["window_handles"] = len(driver.window_handles)
        except Exception as inner:
            driver_state["window_handles"] = f"<unavailable: {inner}>"

        try:
            driver_state["session_id"] = driver.session_id
        except Exception as inner:
            driver_state["session_id"] = f"<unavailable: {inner}>"
...
def log_error_detailed(context, e, driver=None):
    """
    Logs a SHORT, readable line to the console (so the live log stays
    scannable) and appends the FULL diagnostic — real Python traceback +
    driver state — to errors.log for later inspection.

    Use this instead of `log_message(f"... {e}", level="ERROR")` anywhere
    you actually want to know *why* something crashed, not just *that* it did.
    """
    report = diagnose_exception(e, driver=driver, context=context)

    # Short line on screen — this is what you scan live
    log_message(
        f"{context} — {report['exc_type']}: {report['exc_message']} "
        f"(@ {report['location']})",
        level="ERROR",
    )

    # Full detail persisted to disk — this is what you open when you need answers
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    block_lines = [
        "=" * 100,
        f"[{timestamp}] {context}",
        f"Exception : {report['exc_type']}: {report['exc_message']}",
        f"Raised at : {report['location']}",
    ]
    if report["driver_state"]:
        block_lines.append("Driver state at time of failure:")
        for k, v in report["driver_state"].items():
            block_lines.append(f"    {k}: {v}")
    block_lines.append("Traceback:")
    block_lines.append(report["traceback_text"])
    ...
    with _error_log_lock:
        try:
            ...
            # Last resort — at least don't crash the worker because logging failed
            print(f"[!] Could not write to {ERROR_LOG_FILE}: {log_write_err}")
...
def calculate_instagram_engagement_rate(profile_info, post_edges):
    if not isinstance(profile_info, dict) or not isinstance(post_edges, list):
        return None
    ...
    followers = int(user_data.get("follower_count") or 0)
    if followers <= 0:
        return None
    ...
    average_engagement = total_interactions / counted_posts
    return round((average_engagement / followers) * 100, 2)


def calculate_tiktok_engagement_rate(profile_info, video_edges):
    if not isinstance(profile_info, dict) or not isinstance(video_edges, list):
        return None

    stats = profile_info.get("stats", {})
    followers = int(stats.get("follower_count") or 0)
    if followers <= 0:
        return None
    ...
    average_interactions = total_interactions / counted_videos
    return round((average_interactions / followers) * 100, 2)


def save_scrape_to_db(username, url, profile_info, post_info, engagement_rate=None, conn=None):
    own_conn = False
    if conn is None:
        conn = connect_db()
        own_conn = True
    ...
    cur.execute(
        ...
        (username, url, scraped_at, profile_json, post_json, engagement_rate, status)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (
            username,
            ...
            json.dumps(post_info, ensure_ascii=False) if post_info else None,
            engagement_rate,
            "saved",
        ),
    )
...
def load_scrape_from_db(username, conn=None):
    own_conn = False
    if conn is None:
        conn = connect_db()
        own_conn = True
    ...
    return {
        ...
        "profile_info": json.loads(row["profile_json"] or "null"),
        "post_info": json.loads(row["post_json"] or "null") if row["post_json"] else None,
        "engagement_rate": row["engagement_rate"],
        "status": row["status"],
    }


def search_profiles(query, conn=None):
    if not query:
        return []
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT username, url, scraped_at, engagement_rate FROM profiles "
        "WHERE LOWER(username) LIKE ? OR LOWER(profile_json) LIKE ? OR LOWER(post_json) LIKE ?",
...
def ai_search_profiles(query, conn=None, model="local"):
    if model == "local":
        return search_profiles(query, conn=conn)

    # Placeholder for external AI search integration.
    # Replace this block with a call to OpenAI, Azure OpenAI, or another semantic search API.
    return search_profiles(query, conn=conn)


def visualize_profile_data(username, conn=None, output_dir="output"):
    row = load_scrape_from_db(username, conn=conn)
    if not row:
        log_message(f"No database record found for @{username}", level="WARNING")
        return None

    profile_info = row.get("profile_info") or {}
    post_info = row.get("post_info") or {}
    timeline_key = ScraperConfig(SESSION_IDS=[]).TARGET_QUERIES["timeline"]
    edges = []
    if isinstance(post_info, dict):
        edges = post_info.get("data", {}).get(timeline_key, {}).get("edges", [])

    summary = {
        "username": username,
        "url": row.get("url"),
        "scraped_at": row.get("scraped_at"),
        "followers": profile_info.get("data", {}).get("user", {}).get("follower_count"),
        "engagement_rate": row.get("engagement_rate"),
        "posts": len(edges),
    }

    os.makedirs(os.path.join(output_dir, username), exist_ok=True)
    summary_file = os.path.join(output_dir, username, "summary.json")
    with open(summary_file, "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=4)

    if plt and edges:
        likes = [int(e.get("node", {}).get("like_count") or 0) for e in edges]
        comments = [int(e.get("node", {}).get("comment_count") or 0) for e in edges]
        ids = [e.get("node", {}).get("id") or str(idx + 1) for idx, e in enumerate(edges)]

        fig, ax = plt.subplots(figsize=(10, 5))
        ax.bar([f"P{idx+1}" for idx in range(len(ids))], likes, label="Likes", alpha=0.7)
        ax.bar([f"P{idx+1}" for idx in range(len(ids))], comments, bottom=likes, label="Comments", alpha=0.7)
        ax.set_title(f"Engagement for @{username}")
        ax.set_xlabel("Post")
        ax.set_ylabel("Count")
        ax.legend()
        chart_path = os.path.join(output_dir, username, "engagement_chart.png")
        fig.tight_layout()
        fig.savefig(chart_path)
        plt.close(fig)
        summary["chart_path"] = chart_path

    log_message(f"Visualized data for @{username} at {summary_file}", level="SUCCESS")
    return summary


def background_scrape_worker(url, config, db_path=DATABASE_PATH):
    username = get_username(url)
    log_message(f"Background scrape started for @{username}", level="INFO")
    try:
        session_id = config.SESSION_IDS[0] if config.SESSION_IDS else None
        driver = configure_driver(session_id, config)
        if not driver:
            log_message(f"Background scrape failed to create driver for @{username}", level="ERROR")
            return
        ...
        log_error_detailed(context=f"Background error for @{username}", e=e)
    finally:
        try:
            if driver:
                driver.quit()
...
def schedule_background_rescrape(url, config, db_path=DATABASE_PATH):
    thread = threading.Thread(
        target=background_scrape_worker,
        args=(url, config, db_path),
        daemon=True,
    )
    thread.start()
    return thread
...
def load_and_show_profile(username, url, config, db_path=DATABASE_PATH):
    conn = connect_db(db_path)
    row = load_scrape_from_db(username, conn=conn)
    if row:
        log_message(f"Loaded @{username} from database", level="SUCCESS")
        visualization = visualize_profile_data(username, conn=conn)
        conn.close()
        return {"status": "loaded", "row": row, "visualization": visualization}

    conn.close()
    log_message(f"No DB profile for @{username}; scheduling background scrape", level="WARNING")
    schedule_background_rescrape(url, config, db_path=db_path)
    return {"status": "scheduled", "username": username, "url": url}
        ...
        if data["profile_info"]:
            ...
            if data["reel_info"]:
                ...
                if filtered_reel:
                    ...
                    edges = filtered_reel.get("data", {}).get(timeline_key, {}).get("edges", [])
                    if isinstance(edges, list):
                        engagement_rate = calculate_instagram_engagement_rate(profile_to_save, edges)
                        if engagement_rate is None:
                            engagement_rate = calculate_tiktok_engagement_rate(profile_to_save, edges)
    ...
    try:
        while True:
            ...
            try:
                ...
                log_error_detailed(
                    context=f"Worker-{worker_id} error on @{get_username(url)} ({url})",
                    e=e,
                    driver=driver,
                )
                with stats_lock:
