...
LOGO_CONF_THRESHOLD   = 0.50  # min confidence from logo model to call it a logo
...
class Detector:
    ...
    def __init__(self):
        ...
        print("  [1/4] YOLOv8n  (person detection) …", end=" ", flush=True)
        self.yolo = YOLO("yolov8n.pt")
        print("✓")

        # Optional dedicated logo-detection model.
        # Set LOGO_MODEL_PATH env var or place weights at ./logo_detector.pt
        # (e.g. a YOLOv8 model fine-tuned on OpenLogo / LogoDet-3K).
        print("  [2/4] Logo detector …", end=" ", flush=True)
        logo_weights = os.environ.get("LOGO_MODEL_PATH", "logo_detector.pt")
        if os.path.exists(logo_weights):
            try:
                self.logo_yolo = YOLO(logo_weights)
                print("✓")
            ...
            self.logo_yolo = None
            print("✗ (no weights found, skipping — set LOGO_MODEL_PATH)")

        print("  [3/4] FairFace gender model …", end=" ", flush=True)
        ...
        print("  [4/4] FairFace age model …", end=" ", flush=True)
    ...
    def has_logo(self, image_path: str):
        """Return (found: bool, best_confidence: float, label: str|None) using
        the dedicated logo-detection model, if one was loaded."""
        if self.logo_yolo is None:
            return False, 0.0, None
        results = self.logo_yolo(image_path, verbose=False)
        best = 0.0
        best_label = None
        for r in results:
            for box in r.boxes:
                ...
                if conf > best:
                    ...
                    best_label = r.names.get(cls_id, "logo") if hasattr(r, "names") else "logo"
        return best >= LOGO_CONF_THRESHOLD, best, best_label

    # ── main predict ─────────────────────────────────────────────────────────

    def _predict(self, img: Image.Image, tmp_path: str) -> dict:
        """Run person detection then gender/age classification.
        Logo checking is handled separately by the caller (only for creator
        accounts, never business accounts)."""
    ...
    def _load_profile_image(self, username: str, local_dir: str = None):
        """Try local output folder first, then remote URL. Returns PIL Image or None."""
        img = None
        if local_dir:
    ...
    def detect(self, username: str, local_dir: str = None) -> dict:
        ...
        """
        img = self._load_profile_image(username, local_dir)
        if img is None:
            return {
                "gender": "Unknown",
                "age_group": "Unknown",
                ...
                "logo_confidence": 0.0,
                "logo_label": None,
                "detection_status": "Image not found locally or remotely",
            }
    ...
    def check_logo(self, username: str, local_dir: str = None):
        """
        Standalone logo check — call this separately, only for creator accounts
        (account_type 1/3) whose gender came back Unknown. Never call this for
        business accounts.

        Returns (is_logo: bool, logo_confidence: float %, logo_label: str|None).
        """
        img = self._load_profile_image(username, local_dir)
        if img is None or self.logo_yolo is None:
            return False, 0.0, None
        ...
        try:
            with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as f:
                ...
                tmp = f.name
            _register_temp(tmp)
            is_logo, logo_conf, logo_label = self.has_logo(tmp)
            return is_logo, round(logo_conf * 100, 1), logo_label
        except Exception:
            return False, 0.0, None
        finally:
            if tmp:
...
def parse_location_to_address_components(location_name: str, address: str = None, city: str = None) -> Dict:
    ...
    result['country'] = country_to_alpha2(result['country'])
    return result
...
def identify_niche(user_info: dict, posts: Optional[List[dict]] = None) -> dict:
   ...
   "Fashion & Beauty": [
        ...
        "Sports & Outdoors": [
            ...
            "outdoors", "adventure", "hiking", "camping", "backpacking", "trailrunning", "mountaineering", "rockclimbing", "bouldering", "kayaking", "canoeing", "whitewater",

            "rafting", "fishing", "flyfishing", "hunting", "wildlife", "nature", "wilderness", "overlanding", "vanlife", "survivalskills", "bushcraft", "offroad",

            "explore", "wanderlust", "getoutside", "nationalparks", "trails", "outdoorgear", "camplife", "campvibes", "wildernesscamp", "dispersedcamping", "mountainlife", "alpinism",

            "expeditions", "ultralight", "outdoorswoman", "outdoorsman", "surfing", "surflife", "surfer", "wavesurfing", "paddleboarding", "sup", "kayaking", "watersports",

            "rowing", "sailing", "kitesurfing", "windsurfing", "jetski", "wakeboarding", "waterski", "swimming", "openwater", "wildswimming", "freediving", "scubadiving",

            "snorkeling", "underwaterphotography", "snowboarding", "skiing", "skilife", "snowsports", "freeski", "backcountryski", "skitour", "skintrack", "skinning", "snowmobiling",

            "icefishing", "icehockey", "figureskating", "speedskating", "cycling", "cyclinglife", "roadcycling", "roadbike", "velodrome", "criterium", "cyclocross", "gravel",

            "gravelriding", "mtb", "mountainbiking", "enduro", "downhill", "trailbike", "xc", "bikepacking", "touring", "commutecycling", "fixedgear", "singlespeed",

            "velominati", "strava", "zwift", "running", "runner", "runnerslife", "marathon", "halfmarathon", "ultramarathon", "trailrunning", "5k", "10k",

            "parkrun", "track", "crosscountry", "runcommunity", "runnerlife", "VO2max", "heartratezone", "runningshoes", "gps watch", "weightlifting", "powerlifting", "bodybuilding",

            "crossfit", "gymnastics", "basketball", "nba", "soccer", "football", "nfl", "baseball", "mlb", "volleyball", "tennis", "pickleball",

            "badminton", "squash", "golf", "golflife", "cricket", "rugby", "lacrosse", "fieldockey", "waterpolo", "boxing", "mma", "bjj",

            "wrestling", "judo", "karate", "taekwondo", "martialarts", "sportsphotography", "athletelife", "sportscoach", "youthsports", "highschoolsports", "collegesports"

        ],

    "Home & Garden": [

            "homedecor", "interiordesign", "homedesign", "interiorinspo", "homestyle", "roomdecor", "livingroom", "bedroom", "bathroom", "kitchen", "diningroom", "homeoffice",

            "studioapartment", "smallspaces", "rentalfriendly", "affordabledecor", "diydecor", "thrifteddecor", "thriftshopping", "vintagedecor", "bohodecor", "minimalism", "maximalism", "scandi",

            "japandi", "midcenturymodern", "farmhousestyle", "cottagecore", "darkacademia", "grandmillennial", "transitional", "moody", "colorfulinteriors", "neutralpalette", "biophilicdesign", "archinteriors",

            "interiordesigner", "stager", "homerenovation", "diy", "diyproject", "diyhome", "handyman", "fixerupper", "houseflipping", "beforeandafter", "renovation", "remodel",

            "kitchenremodel", "bathroomremodel", "tileinstall", "flooring", "hardwoodflooring", "laminateflooring", "concretefloors", "painting", "colorpalette", "wallpaper", "shiplap", "beadboard",

            "crownmolding", "wainscoting", "cabinetpainting", "cabinetrefacing", "kitchenupgrade", "lightingupgrade", "plumbing", "electrical", "smartupgrade", "landscaping", "curb appeal", "outdoorpatio",

            "deck", "pergola", "fencing", "poollife", "homeorganization", "organize", "declutter", "kondo", "sparksjoymariakondo", "storagesolutions", "closetorganization", "capsulewardrobe",

            "kitchenorganization", "pantryorganization", "laundrytips", "mudroom", "tinyhouse", "tinyhome", "smallhomelife", "cleaning", "cleaningtips", "cleantok", "deepcleaning", "naturalclean",

            "cleaninghacks", "zerowastecleaning", "homemaintenance", "diyrepairs", "preventivemaintenance", "gardening", "garden", "gardenlife", "plantlover", "plantmom", "plantdad", "houseplants",

            "indoorplants", "tropicalplants", "succulents", "cacti", "plantcare", "propagation", "urbangardening", "growyourown", "kitchengarden", "herbgarden", "vegetablegarden", "raisedgarden",

            "raiseds beds", "compost", "composting", "soilhealth", "permaculture", "organicgardening", "seedstarting", "seedsaving", "flowerfarming", "cottagegarden", "wildlifegarden", "pollinatorgarden",

            "beekeeping", "chickens", "backyardchickens", "foodforest", "aquaponics", "hydroponics", "microgreens"

        ],

    "Korean Beauty": [

            "kbeauty", "koreanbeauty", "koreanskincare", "koreanskincareroutine", "kskincare", "kbeautyroutine", "kbeautyaddict", "kbeautylover", "koreancosmetics", "koreanbrand", "kbeautyreview", "skincarekorea",

            "abcofkbeauty", "glasskin", "glassskinroutine", "cloudskin", "dumplingkin", "chokchok", "slugging", "doublecleanse", "layering", "essences", "fermentedkbeauty", "skinbarrier",

            "skincycling", "skintok", "skincareobsessed", "skincareaddicts", "skincarejunkie", "sheetmask", "slimask", "sleeping mask", "patchs", "eyecream", "ampule", "serum",

            "essence", "toner", "mist", "emulsion", "gel cream", "sunscreen", "spf", "uvauvb", "vitaminc", "niacinamide", "hyaluronicacid", "ceramides",

            "aha", "bha", "pha", "retinol", "snailmucin", "centella", "galactomyces", "propolis", "ginseng", "green tea", "bakuchiol", "azelaicacid",

            "cica", "mugwort", "licorice root", "niacinamide", "tranexamic", "exfoliant", "moisturizer", "oilcleanser", "foamcleanser", "gelcleanser", "micellar", "koreammakeup",

            "koreanlook", "nomakelook", "clean girl", "jamskin", "strawberrymakeup", "dewyblush", "auroramakeup", "puppyliner", "cat eye", "koreaneyeliner", "koreanlips", "gloss lips",

            "ombre lips", "monochrome", "cushion", "bbcream", "ccream", "blush", "gradient blush", "undereye", "koreanskincaremakeup", "tinted moisturiser", "kbeautyhaul", "kbeautycommunity",

            "skincarecorea", "koreanskincaretips", "koreancleansing", "koreanbodycare", "exfoliatingkorea", "italytowel", "cosrx", "somebymi", "innisfree", "laneige", "missha", "skinfood",

            "etudehouse", "facerepublic", "roundlab", "anua", "mediheal", "pyunkang yul", "beauty of joseon", "isntree", "abib", "klairs", "torriden", "mixsoon",

            "apliu", "9wishes", "dr.jart", "iope", "sulwhasoo", "history of whoo", "ohui", "hanyul", "amorepacific", "laneige", "medicube", "skin1004",

            "masil", "d'alba", "numbuzin", "tirtir", "romand", "clio", "peripera", "wakemake", "twelvenyc", "hold live"

        ],

    "Apps & Digital Services": [

            "productivity", "productivityhacks", "productivitytips", "gettingthingsdone", "gtd", "secondbrain", "pkm", "notiontemplate", "notion", "obsidian", "roamresearch", "logseq",

            "todoist", "anydo", "ticktick", "asana", "trello", "clickup", "monday", "airtable", "coda", "makecom", "zapier", "automation",

            "nocode", "lowcode", "workflow", "tools", "techtools", "digitalnomad", "remotework", "workfromhome", "wfh", "homeoffice", "digitalsetup", "desktopsetup",

            "productivemorning", "deepwork", "focusapp", "timeblocking", "calendar", "scheduling", "ai", "artificialintelligence", "chatgpt", "claude", "gpt", "llm",

            "aitools", "aiart", "imagegen", "midjourney", "stablediffusion", "dalle", "sora", "aiwriting", "aiassistant", "promptengineering", "AIworkflow", "agentai",

            "aiproductivity", "aiforcreators", "notebooklm", "perplexity", "gemini", "copilot", "elevenlabs", "heygen", "runway", "pika", "kling", "luma",

            "ainocode", "aiautomation", "aibusiness", "aistartup", "creatortools", "contentstrategy", "contentcreation", "socialmediatools", "schedulingtools", "buffer", "hootsuite", "later",

            "linkinbio", "contentcalendar", "videocreation", "videoediting", "capcut", "canva", "figma", "adobeexpress", "screenstudio", "loom", "descript", "podcasttools",

            "streamlabs", "obs", "davinci", "premiere", "aftereffects", "reelstips", "tiktokgrowth", "youtubegrowth", "instagramgrowth", "creatoreconomy", "ugc", "branddeals",

            "sponsorships", "affiliate", "fintechapps", "budgetingapp", "ynab", "personalfinance", "investingapps", "cryptoapps", "defi", "web3", "subscriptions", "cashbackapps",

            "rewardapps", "loyalty", "streamingservices", "netflix", "spotify", "applemusic", "youtubepremium", "podcast", "audiobook", "kindle", "gaming", "mobileapp",

            "gamedevelopment", "appstore", "googleplay", "androidapps", "iosapps", "appoftheday", "appreview", "softwarereview", "saas", "techreview", "apprecommen", "toolstack",

            "techstack", "productreview"],
    ...
    for _p in sorted_by_time:
        ...
        try:
            ...
            if _n.get('product_type') == 'clips' and _n.get('code'):
                reel_links.append(f"https://www.instagram.com/p/{_n['code']}/embed")
        except (AttributeError, TypeError, KeyError):
            continue
    return {
        'portfolio_video_urls': reel_links
    }
...
# ── social links ──────────────────────────────────────────────────────────
    ...
    primary_social_link = f"https://www.instagram.com/{username}" if username else None
    ...
    for link_obj in bio_links:
        ...
        url = link_obj.get('url', '')
        if url:
            other_social_links.append(url)

    # ── creator_location_from_bio ─────────────────────────────────────────────
    creator_location_from_bio = {
        'address':   None,
        'city':      bio_location.get('city') or None,
        'country':   country_to_alpha2(bio_location.get('country')),
    ...
    if bio_city:
        for loc in location_analysis.get('all_locations', []):
            loc_name = loc.get('name') or ''   # name can be present-but-None
            if loc.get('city') == bio_city or loc_name.startswith(bio_city):
    ...
    creator_location_from_csv = {
        ...
        'country':   country_to_alpha2(csv_location.get('country')),
    ...
    return {
         ...
         **reels ,
        'primary_social_link':        primary_social_link,
        'other_social_links':         other_social_links,
...
def analyze_creator_data(creator_dir: str, gender_lookup: dict = None) -> Optional[dict]:
    ...
    try:
        ...
        # ── address and lat/lng: bio preferred, CSV as fallback ───────────────
        # Location comes from the bio and the CSV row only — never from post geotags.
        _csv_loc = csv_location

        def _first(*values):
            """First value that is neither None nor an empty/whitespace string.
            Unlike `or`, this keeps a legitimate 0 / 0.0 (valid lat/lng)."""
            for v in values:
                if v is None:
                    continue
        ...
        bio_address_city    = _first(bio_location.get('city'),    _csv_loc.get('city'))
        bio_address_state   = _first(bio_location.get('state'),   _csv_loc.get('state'))
        bio_address_country = country_to_alpha2(_first(bio_location.get('country'),
                                                       _csv_loc.get('country')))
        bio_address_zip     = _first(bio_location.get('zip_code'),
                                     _csv_loc.get('zip_code'),
                                     _csv_loc.get('postal_code'),
                                     _csv_loc.get('post_code'))

        bio_lat = _first(bio_location.get('latitude'), bio_location.get('lat'))
        bio_lng = _first(bio_location.get('longitude'), bio_location.get('lng'))

        # Only inherit the CSV row's coordinates when the resolved city actually came
        # from that row. Otherwise a bio-derived city (say Hamburg) would be pinned to
        # the CSV row's coordinates (say Munich's).
        _csv_city = _csv_loc.get('city')
        if bio_lat is None and (not bio_address_city
                                or (_csv_city and bio_address_city == _csv_city)):
            bio_lat = _first(_csv_loc.get('latitude'), _csv_loc.get('lat'))
            bio_lng = _first(_csv_loc.get('longitude'), _csv_loc.get('lng'))
        if bio_lat is None and bio_address_city:
            for _loc in location_analysis.get('all_locations', []):
                _lname = _loc.get('name') or ''       # name can be present-but-None
                if _loc.get('city') == bio_address_city or _lname.startswith(bio_address_city):
                    bio_lat = _loc.get('lat')
                    bio_lng = _loc.get('lng')
                    break
        bio_latitude  = bio_lat
        bio_longitude = bio_lng

        # Bio-only view of the location — same shape as the one in the AI entry, so
        # both sources are available side by side on the analyzed record.
        creator_location_from_bio = {
            'address':   None,
            'city':      bio_location.get('city') or None,
            'country':   country_to_alpha2(bio_location.get('country')),
        ...
        if creator_location_from_bio['city']:
            for _loc in location_analysis.get('all_locations', []):
                _lname = _loc.get('name') or ''
                if (_loc.get('city') == creator_location_from_bio['city']
                        or _lname.startswith(creator_location_from_bio['city'])):
                    creator_location_from_bio['latitude']  = _loc.get('lat')
                    creator_location_from_bio['longitude'] = _loc.get('lng')
                    break

        # ── account_type: 2 = Business, 1/3 = Creator ────────────────────────
        # Exception: a business account whose profile photo resolves to a clear
        # Male/Female (not Unknown/logo) is treated as a creator for output routing.
        raw_account_type = user_info.get('data', {}).get('user', {}).get('account_type', 1)
        if raw_account_type == 2 and resolved_gender in ('Male', 'Female'):
            account_type = 1  # route into creator files instead of business files
        else:
        ...
        analyzed_entry = {
            ...
            # gender/age omitted only when routed to business output (account_type 2, Unknown gender)
            **({} if account_type == 2 else {
            ...
            # is_logo reported for every account type — business accounts are
            # exactly where a logo profile pic is most likely
            'is_logo':                     gender_detection.get('is_logo', False),
            'logo_confidence':             gender_detection.get('logo_confidence', 0.0),
            'logo_label':                  gender_detection.get('logo_label'),
            'email':                       email_info.get('email'),
            ...
            'creator_location_from_csv': {
                ...
                'country':   country_to_alpha2(_csv_loc.get('country')),
...
# ══════════════════════════════════════════════════════════════════════════════
# JSONL SANITIZER  (integrated from jsonlfixerfullname.py)
#
# Applies to the AI-facing .jsonl streams only. The wide _data.json files are
# written exactly as before — untouched, unfiltered.
# ══════════════════════════════════════════════════════════════════════════════

# Keywords stripped from a username before using it as a fallback full_name
REMOVE_KEYWORDS = {"official", "iam", "i_am", "real", "the", "its", "itsme", "mr", "ms", "mrs", "dr"}

# Minimum post count for a record to stay in the main .jsonl output
MIN_POST_COUNT = 5


def clean_username_to_full_name(username: str) -> str:
    """'iam_ram.official99' → 'Ram'. Used only to backfill a blank full_name."""
    cleaned = re.sub(r"[^a-zA-Z\s]", " ", username or "")
    words = [
        w for w in cleaned.split()
        if w.lower() not in REMOVE_KEYWORDS and w
    ]
    return " ".join(w.capitalize() for w in words)


def try_fix_json(line: str):
    """
    Attempt multiple repair strategies in order, returning the first that parses.
    Returns (record_dict, strategy_name) or (None, None).
    """
    original = line
    strategies = []

    # Strategy 1: strip control characters only (least invasive)
    s1 = re.sub(r"[\x00-\x1f\x7f]", " ", original)
    strategies.append(("strip_control_chars", s1))

    # Strategy 2: fix punctuation issues
    s2 = s1
    s2 = re.sub(r",\s*,", ",", s2)           # double commas
    s2 = re.sub(r",\s*([}\]])", r"\1", s2)   # trailing commas before } or ]
    strategies.append(("fix_punctuation", s2))

    # Strategy 3: safe single-quote fix (only quote delimiters, not apostrophes mid-word)
    s3 = re.sub(r"(?<!\w)'|'(?!\w)", '"', s2)
    strategies.append(("fix_single_quotes_safe", s3))

    # Strategy 4: aggressive single-quote replace (fallback)
    s4 = s2.replace("'", '"')
    strategies.append(("fix_single_quotes_aggressive", s4))

    # Strategy 5: extract JSON object — strips trailing garbage after last }
    match = re.search(r'(\{.*\})', original, re.DOTALL)
    if match:
        s5 = re.sub(r"[\x00-\x1f\x7f]", " ", match.group(1))
        s5 = re.sub(r",\s*([}\]])", r"\1", s5)
        strategies.append(("extract_json_object", s5))
...
def validate_ai_record(record) -> tuple:
    """
    Validate that the record is a proper non-empty JSON object.
    Returns (is_valid, reason).
    """
    if not isinstance(record, dict):
        return False, f"expected object, got {type(record).__name__}"

    if not record:
        return False, "empty object {}"

    # posts must be a list if present
    posts = record.get("posts")
    if posts is not None and not isinstance(posts, list):
        return False, f"'posts' is {type(posts).__name__}, expected list"

    return True, ""


def get_post_count(record: dict) -> int:
    """
    Robustly count valid posts in a record.
    - Handles missing, null, non-list posts → 0
    - Only counts items that are non-null dicts (actual post objects)
    - Ignores nulls, strings, numbers, empty dicts inside the list
    - Deduplicates by post_link if present, so duplicate posts aren't double-counted
    """
    posts = record.get("posts")

    # Not present or null or wrong type → 0
    ...
    for post in posts:
        # Must be a non-empty dict to be a valid post
        if not isinstance(post, dict) or not post:
            continue

        # Deduplicate by post_link if available
...
def _serialize_record(record):
    """
    Serialize an in-memory record to a JSONL line.
    Returns (line, note) — note is non-empty when a fallback encoder was needed.
    Returns (None, reason) if the record cannot be serialized at all.
    """
    try:
        ...
        pass
    try:
        return json.dumps(record, ensure_ascii=False, default=str), "coerced non-JSON values via str()"
    except (TypeError, ValueError) as e:
        return None, f"unserializable record: {type(e).__name__}: {e}"


def sanitize_ai_records(ai_list: List[dict], label: str, reject_sink: List[str]) -> tuple:
    """
    Run every AI-facing record through the jsonlfixerfullname pipeline:

        serialize → re-parse (repair if needed) → validate → backfill full_name
        → route by post count

    Records with >= MIN_POST_COUNT posts are returned as good lines. Low-post and
    unfixable/invalid records are appended to reject_sink (shared between the
    creator and business streams so they land in one _low_bad.jsonl).

    Returns (good_lines, stats_dict).
    """
    good_lines: List[str] = []
    stats = {'total': 0, 'auto_fixed': 0, 'bad': 0, 'low_post': 0, 'name_fixed': 0}

    for idx, raw_record in enumerate(ai_list, start=1):
        line, note = _serialize_record(raw_record)
        if line is None:
            print(f"  {Fore.RED}❌ [{label}] record {idx} unfixable: {note}{Style.RESET_ALL}")
            reject_sink.append(json.dumps(
                {"_reject_reason": note, "_reject_source": label, "_raw": repr(raw_record)[:2000]},
                ensure_ascii=False
            ))
            stats['bad'] += 1
            continue
        if note:
            print(f"  {Fore.YELLOW}⚠  [{label}] record {idx}: {note}{Style.RESET_ALL}")

        # 1. Re-parse to prove the emitted line is valid JSONL
        try:
            record = json.loads(line)
        except json.JSONDecodeError as e:
            # 2. Try progressive repair strategies
            record, strategy = try_fix_json(line)
            if record is not None:
                stats['auto_fixed'] += 1
                print(f"  {Fore.GREEN}✅ [{label}] auto-fixed record {idx} via '{strategy}'{Style.RESET_ALL}")
            else:
                print(f"  {Fore.RED}❌ [{label}] unfixable record {idx}: {e}{Style.RESET_ALL}")
                reject_sink.append(line)
                stats['bad'] += 1
                continue

        # 3. Validate the parsed record
        valid, reason = validate_ai_record(record)
        if not valid:
            print(f"  {Fore.YELLOW}⚠  [{label}] invalid record {idx}: {reason}{Style.RESET_ALL}")
            reject_sink.append(
                json.dumps(record, ensure_ascii=False) if isinstance(record, dict) else line
            )
            stats['bad'] += 1
            continue
        ...
        if not str(full_name).strip():
            ...
            record["full_name"] = clean_username_to_full_name(username)
            stats['name_fixed'] += 1

        # 5. Route by post count
        out_line = json.dumps(record, ensure_ascii=False)
        if get_post_count(record) < MIN_POST_COUNT:
            reject_sink.append(out_line)
            stats['low_post'] += 1
        else:
            good_lines.append(out_line)

    return good_lines, stats


# ══════════════════════════════════════════════════════════════════════════════
# CSV WRITER  (integrated from csvmaker.py)
#
# Builds the CSV straight from the in-memory analyzed records, so no ijson
# streaming / temp-file round trip is needed. Headers and row order are identical
# to csvmaker.py's output.
# ══════════════════════════════════════════════════════════════════════════════

def process_first_name(first_name: str) -> str:
    """Clean first name: strip non-letters/spaces, trim, title-case each word.
    Matches AppScript cleanFirstNames() logic exactly."""
    if not first_name:
        return ''
...
def _pick(*values):
    """First value that is neither None nor an empty/whitespace string.
    Keeps a legitimate 0 / 0.0 (valid latitude/longitude)."""
    for v in values:
        if v is None:
            continue
        if isinstance(v, str) and not v.strip():
...
def resolve_csv_address(creator: dict) -> dict:
    """
    Resolve the address columns for one CSV row.

    Order: merged address_* fields → bio → CSV row. Bio wins over CSV, matching
    the analyzer. Post geotags are never used as a location source.

    csvmaker.py looked up 'address_city'/'address_state'/'address_country'/
    'address_zip' inside creator_location_from_csv and creator_location_from_bio,
    but those sub-dicts use 'city'/'state'/'country'/'zip_code' — so every fallback
    silently returned nothing and the columns came out blank whenever the merged
    address_* fields were empty. This reads the real key names.
    """
    ...
    def tier(*names):
        out = []
        for src in (bio_loc, csv_loc):
            for n in names:
                out.append(src.get(n))
        return out

    city    = _pick(creator.get('address_city'),    *tier('city'))
    state   = _pick(creator.get('address_state'),   *tier('state'))
    country = _pick(creator.get('address_country'), *tier('country'))
    zip_    = _pick(creator.get('address_zip'),     *tier('zip_code', 'postal_code', 'post_code'))

    # Coordinates: never inherit the CSV row's lat/lng for a city that did not come
    # from that row, or the point lands in the wrong place.
    csv_city = csv_loc.get('city')
    coord_sources = [creator.get('latitude'), bio_loc.get('latitude'), bio_loc.get('lat')]
    if not city or (csv_city and city == csv_city):
        coord_sources += [csv_loc.get('latitude'), csv_loc.get('lat')]
    lat = _pick(*coord_sources)

    coord_sources = [creator.get('longitude'), bio_loc.get('longitude'), bio_loc.get('lng')]
    if not city or (csv_city and city == csv_city):
        coord_sources += [csv_loc.get('longitude'), csv_loc.get('lng')]
    lng = _pick(*coord_sources)

    return {
        'city':      city    if city    is not None else '',
        'state':     state   if state   is not None else '',
        'country':   country if country is not None else '',
        'zip':       zip_    if zip_    is not None else '',
        'latitude':  lat     if lat     is not None else '',
        'longitude': lng     if lng     is not None else '',
    }


def build_csv_headers(is_business: bool = False) -> List[str]:
    headers = [
        "email", "primary_social_link", "username", "pk", "first_name", "last_name", "creator_type",
        ...
        *([] if is_business else ["age_group", "age", "gender"]),
    ...
    headers += [
        "price_usd", "time_greater_than_5_minutes", "time_1_to_5_minutes",
        "time_60_seconds", "time_30_seconds", "time_15_seconds",
        "full_name"
    ]
    return headers


def build_csv_row(creator: dict, is_business: bool = False) -> List:
    username  = creator.get('username', '') or ''
    user_pk   = creator.get('user_pk') or creator.get('pk', '')
    full_name = creator.get('full_name', '')

    # Trust first_name / last_name computed upstream (SSA-validated).
    ...
    addr = resolve_csv_address(creator)
    ...
    # Brand logo only for confirmed paid_partnership AND brand != creator's own
    # username (owner/coauthor entries are regular users).
    top_collaboration_names = []
    top_collaboration_brand_logo_list = []
    creator_username_lower = username.lower()
    for collab in creator.get('top_collaboration', []) or []:
    ...
    niche_primary = (creator.get('niche_primary')
                     or (creator.get('niche_data') or {}).get('overall_niche', '')
                     or 'Others')
    niche_secondary = ''
    ...
    other_social_media = " | ".join(l for l in [tiktok_link, youtube_link, x_link, linktree_link] if l)
    ...
    combined_hashtags_str = ' | '.join(str(i) for i in combined_hashtags_list if i)
    ...
    combined_mentions_str = ' | '.join(str(i) for i in combined_mentions_list if i)
    ...
    if isinstance(hashtags_last_90_days_dict, dict):
        hashtags_last_90_days = ' | '.join(f"{t}:{c}" for t, c in hashtags_last_90_days_dict.items())
    else:
    ...
    if isinstance(mentions_last_90_days_dict, dict):
        mentions_last_90_days = ' | '.join(f"{m}:{c}" for m, c in mentions_last_90_days_dict.items())
    else:
        mentions_last_90_days = str(mentions_last_90_days_dict)

    row = [
        email, primary_social_link, username, user_pk, processed_first_name, processed_last_name, creator_type,
        ...
        *([] if is_business else [age_group, age, gender]),
        ...
        combined_hashtags_count, combined_mentions_count,
        hashtags_last_90_days, mentions_last_90_days,
        creator.get('analyzed_date', ''), creator.get('source', ''),
        creator.get('total_collaborations', 0), creator.get('ugc_examples', ''),
        creator.get('latest_post_link', ''), creator.get('latest_post_date', ''),
        creator.get('scraped_date', ''), creator.get('analyzed_date', '')
    ]

    # 25 posts from the posts[] array
    posts = creator.get('posts', []) or []
    for j in range(25):
        if j < len(posts) and isinstance(posts[j], dict):
            p = posts[j]
            ...
            row += [
                ...
                ' | '.join(p.get('mentions', []) or []),
                loc.get('address', '') or '',
                loc.get('city', '') or '',
                loc.get('state', '') or '',
                loc.get('country', '') or '',
                loc.get('latitude', '') if loc.get('latitude') is not None else '',
                loc.get('longitude', '') if loc.get('longitude') is not None else '',
            ]
        else:
            row += [''] * 14

    # 6 pricing/timing columns (reserved) + full_name last
    row += ['', '', '', '', '', '', full_name]
    return row


def write_analyzed_csv(analyzed_list: List[dict], output_csv_path: str, is_business: bool = False) -> tuple:
    """
    Write the analyzed records to CSV, sorted by average_engagement_rate desc.
    Returns (success, rows_written).
    """
    try:
        ordered = sorted(
            analyzed_list,
            key=lambda c: c.get('average_engagement_rate', 0) or 0,
            reverse=True
        )
        ...
        with open(output_csv_path, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            writer.writerow(build_csv_headers(is_business))
            for creator in ordered:
                try:
                    row = build_csv_row(creator, is_business)
                except Exception as e:
                    print(f"{Fore.RED}  Row failed for "
                          f"'{creator.get('username', '?')}': {type(e).__name__}: {e}{Style.RESET_ALL}")
                    continue
                # address_city sits at a fixed index in both header variants
                if not row[7] and not row[9]:
                    blank_address += 1
                cleaned_row = [str(i).replace(',', '') if isinstance(i, str) else i for i in row]
                writer.writerow(cleaned_row)
                rows_written += 1

        if blank_address:
            print(f"{Fore.YELLOW}  {blank_address}/{rows_written} rows have no city and no country "
                  f"(no CSV row, no bio location, no post geotags){Style.RESET_ALL}")
        return True, rows_written

    except Exception as e:
        print(f"{Fore.RED}Error writing {output_csv_path}: {type(e).__name__}: {e}{Style.RESET_ALL}")
        return False, 0
...
def main():
    ...
    low_bad_output_file     = f"{project_name}_low_bad.jsonl"
    print(f"{Fore.CYAN}Creator outputs : {analyzed_output_file}  &  {ai_analyzed_output_file}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}Business outputs: {project_name}_business_data.json  &  {project_name}_business.jsonl{Style.RESET_ALL}")
    print(f"{Fore.CYAN}Rejects (< {MIN_POST_COUNT} posts + unfixable, both streams): {low_bad_output_file}{Style.RESET_ALL}")
    _today = datetime.datetime.now().strftime('%Y%m%d')
    print(f"{Fore.CYAN}CSV outputs     : output{_today}.csv  &  output{_today}_business.csv{Style.RESET_ALL}")
    ...
    if skip_gender:
        ...
        detector = Detector()
        logo_creators: List[tuple] = []   # (creator_folder, username) pairs flagged as logos
        ...
        for idx, creator_folder in enumerate(tqdm(
            ...
            # Determine account_type to decide whether logo-check applies (creators only)
            try:
                ui_check = load_json_file(user_info_path) if os.path.exists(user_info_path) else {}
                acct_type_check = ui_check.get('data', {}).get('user', {}).get('account_type', 1)
            except Exception:
                acct_type_check = 1
            ...
            # Logo check ONLY applies to creator accounts (1/3), and only when
            # gender came back Unknown. Business accounts (2) never get this check —
            # for business accounts, Unknown gender just means "stays business as usual".
            if acct_type_check != 2 and result.get('gender') == 'Unknown':
                is_logo, logo_conf, logo_label = detector.check_logo(username, local_dir=base_path)
                result['is_logo'] = is_logo
                result['logo_confidence'] = logo_conf
                result['logo_label'] = logo_label
                if is_logo:
                    status_str += f"  → flagged as LOGO ({logo_label}, {logo_conf}%), excluding from analysis"
                    logo_creators.append((creator_folder, username))
        ...
        if logo_creators:
            logo_folders_set = {folder for folder, _ in logo_creators}
            creators_to_analyze = [c for c in creators_to_analyze if c not in logo_folders_set]

            logo_csv_path = "logo_usernames.csv"
            try:
                with open(logo_csv_path, 'w', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    writer.writerow(["username"])
                    for _, uname in logo_creators:
                        writer.writerow([uname])
                print(f"{Fore.YELLOW}Excluded {len(logo_creators)} logo accounts — saved to: {logo_csv_path}{Style.RESET_ALL}")
            except Exception as e:
                print(f"{Fore.RED}Error saving {logo_csv_path}: {e}{Style.RESET_ALL}")

    phase_label = "Analysis" if skip_gender else "Phase 2: Parallel analysis"
    print(f"\n{Fore.YELLOW}{phase_label} starting...{Style.RESET_ALL}")

    all_analyzed_results    = []      # account_type 1 & 3
    all_ai_analyzed_results = []
    biz_analyzed_results    = []      # account_type 2
    biz_ai_analyzed_results = []
    failed_usernames        = []
        ...
        for future in tqdm(
            ...
            try:
                ...
                if result:
                    ...
                    if acct_type == 2:
                        biz_analyzed_results.append(result['analyzed'])
                        biz_ai_analyzed_results.append(result['ai_analyzed'])
                    else:
    ...
    creator_count = len(all_analyzed_results)
    biz_count     = len(biz_analyzed_results)

    # Rejects from BOTH the creator and business .jsonl streams collect here
    low_bad_lines: List[str] = []
    sanitize_stats = {}

    def _save_json_and_jsonl(analyzed_list, ai_list, json_path, jsonl_path, label):
        if not analyzed_list:
            print(f"{Fore.YELLOW}No {label} results to save.{Style.RESET_ALL}")
            return
        final_output = {
        ...
        # ── wide _data.json: written untouched, no filtering ──────────────────
        try:
            with open(json_path, 'w', encoding='utf-8') as f:
                json.dump(final_output, f, indent=2, ensure_ascii=False)
            print(f"{Fore.GREEN}[{label}] JSON saved → {json_path}  ({len(analyzed_list)} entries){Style.RESET_ALL}")
        except Exception as e:
            print(f"{Fore.RED}Error saving {json_path}: {e}{Style.RESET_ALL}")

        # ── AI-facing .jsonl: run through the jsonlfixer pipeline ────────────
        good_lines, stats = sanitize_ai_records(ai_list, label, low_bad_lines)
        sanitize_stats[label] = stats
        try:
            with open(jsonl_path, 'w', encoding='utf-8') as f:
                for out_line in good_lines:
                    f.write(out_line + '\n')
            print(f"{Fore.GREEN}[{label}] JSONL saved → {jsonl_path}  "
                  f"({len(good_lines)} clean of {len(ai_list)}; "
                  f"{stats['low_post']} low-post, {stats['bad']} bad, "
                  f"{stats['auto_fixed']} auto-fixed, {stats['name_fixed']} names filled){Style.RESET_ALL}")
        except Exception as e:
            print(f"{Fore.RED}Error saving {jsonl_path}: {e}{Style.RESET_ALL}")

    # ── Creator files (account_type 1 & 3) ───────────────────────────────────
    _save_json_and_jsonl(
        all_analyzed_results,
        all_ai_analyzed_results,
        analyzed_output_file,
        ai_analyzed_output_file,
        "Creator (type 1&3)"
    )

    # ── Business files (account_type 2) ──────────────────────────────────────
    biz_analyzed_file    = f"{project_name}_business_data.json"
    biz_ai_analyzed_file = f"{project_name}_business.jsonl"
    _save_json_and_jsonl(
        biz_analyzed_results,
        biz_ai_analyzed_results,
        biz_analyzed_file,
        biz_ai_analyzed_file,
        "Business (type 2)"
    )

    # ── Combined low-post + bad file (creator AND business rejects) ──────────
    if low_bad_lines:
        try:
            with open(low_bad_output_file, 'w', encoding='utf-8') as f:
                for out_line in low_bad_lines:
                    f.write(out_line + '\n')
            print(f"{Fore.YELLOW}[Rejects] JSONL saved → {low_bad_output_file}  "
                  f"({len(low_bad_lines)} entries){Style.RESET_ALL}")
        except Exception as e:
            print(f"{Fore.RED}Error saving {low_bad_output_file}: {e}{Style.RESET_ALL}")
    else:
        print(f"{Fore.GREEN}No low-post or bad records — {low_bad_output_file} not created.{Style.RESET_ALL}")

    # ── CSV export (integrated from csvmaker.py) ─────────────────────────────
    today_date       = datetime.datetime.now().strftime('%Y%m%d')
    creator_csv_file = f"output{today_date}.csv"
    biz_csv_file     = f"output{today_date}_business.csv"

    if all_analyzed_results:
        print(f"\n{Fore.CYAN}Writing creator CSV...{Style.RESET_ALL}")
        ok, n = write_analyzed_csv(all_analyzed_results, creator_csv_file, is_business=False)
        if ok:
            print(f"{Fore.GREEN}[Creator (type 1&3)] CSV saved → {creator_csv_file}  ({n} rows){Style.RESET_ALL}")
        else:
            print(f"{Fore.RED}✗ Creator CSV conversion failed!{Style.RESET_ALL}")

    if biz_analyzed_results:
        print(f"\n{Fore.CYAN}Writing business CSV...{Style.RESET_ALL}")
        ok, n = write_analyzed_csv(biz_analyzed_results, biz_csv_file, is_business=True)
        if ok:
            print(f"{Fore.GREEN}[Business (type 2)] CSV saved → {biz_csv_file}  ({n} rows){Style.RESET_ALL}")
        else:
            print(f"{Fore.RED}✗ Business CSV conversion failed!{Style.RESET_ALL}")

    total_low  = sum(s['low_post'] for s in sanitize_stats.values())
    total_bad  = sum(s['bad'] for s in sanitize_stats.values())
    total_fix  = sum(s['auto_fixed'] for s in sanitize_stats.values())
    total_name = sum(s['name_fixed'] for s in sanitize_stats.values())

    print(f"\n{Fore.CYAN}Summary — Creator: {creator_count}  |  Business: {biz_count}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}JSONL sanitizer — auto-fixed: {total_fix}  |  names filled: {total_name}  "
          f"|  low-post (< {MIN_POST_COUNT}): {total_low}  |  bad: {total_bad}{Style.RESET_ALL}")
